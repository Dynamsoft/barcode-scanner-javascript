<script lang="ts">
  ...
  onMount(() => {
    ...
    const config: BarcodeScannerConfig = {
      ...
      uiPath:
        "https://cdn.jsdelivr.net/npm/dynamsoft-barcode-reader-bundle@11.4.3000/dist/ui/barcode-scanner.ui.xml",
    ...
    (async () => {
      try {
        ...
        console.error("Failed to launch barcode scanner:", error);
        alert("Failed to start camera: " + (error as Error).message);
      }
    })();
